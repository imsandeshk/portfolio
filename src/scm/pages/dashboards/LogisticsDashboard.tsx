...
const LogisticsDashboard: React.FC = () => {
  ...
  const { data: shipments } = useQuery({
    ...
    queryFn: () => MockApi.listShipments({ role: "logistics", userId: currentUser!.id }),
    enabled: !!currentUser,
  });
          ...
          <div className="space-y-3">
            {shipments?.map(s => (
              <div key={s.id} className="flex items-center justify-between border rounded p-3">
                ...
                <Button asChild size="sm"><Link to={`/scm/shipments/${s.id}`}>Update</Link></Button>
              </div>
            ))}
            {!shipments?.length && <div className="text-sm text-muted-foreground">No shipments assigned.</div>}
          </div>
        </CardContent>
      </Card>
      <Card>
        ...
        <CardContent>
          <div className="h-48 grid place-items-center text-sm text-muted-foreground border rounded">Map Placeholder</div>
        </CardContent>
      </Card>
    </div>
  );
