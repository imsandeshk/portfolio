...
const TabSwitcher: React.FC<TabSwitcherProps> = ({ 
  ...
  return (
    <div className="flex flex-wrap justify-center gap-4 mb-10">
      <div className={`backdrop-blur-md bg-white/5 p-1.5 rounded-2xl border border-white/10 shadow-[0_8px_32px_rgba(0,0,0,0.4)] ${isMobile ? 'w-full flex justify-between' : ''}`}>
        {tabs.map((tab) => {
          ...
          return (
            <motion.button
              ...
              className={`
                relative px-3 sm:px-6 py-2 sm:py-3 rounded-xl text-xs sm:text-sm font-medium transition-all
